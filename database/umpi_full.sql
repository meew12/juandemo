-- ════════════════════════════════════════════════════════════
--  UMPI Marketplace — Volcado COMPLETO (Esquema + Datos)
--  Importar con phpMyAdmin en una base MySQL vacía
--  Tablas: ~20 | Registros: usuarios, publicaciones, planes, etc.
-- ════════════════════════════════════════════════════════════
--
--  CÓMO USAR:
--    1) En GoDaddy cPanel, crea una base de datos MySQL vacía
--       (por ejemplo: umpidata) y un usuario con todos los privilegios.
--    2) Abre phpMyAdmin → selecciona la base vacía → pestaña "Importar".
--    3) Sube este archivo (umpi_full.sql) y pulsa "Continuar".
...
--    · CREATE TABLE + ADD FOREIGN KEY  (esquema, ~20 tablas)
