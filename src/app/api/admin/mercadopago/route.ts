...
export async function GET() {
  try {
    ...
    const rows = await findSiteConfigByKeys([...MP_KEYS]);
  ...
  try {
    ...
    for (const [key, value] of entries) {
      if (value === null) {
        ...
        await upsertSiteConfig(key, value.trim());
        saved++;
      }
    ...
    return NextResponse.json({
      ...
      saved,
      cleared,
      message: `${saved} credencial(es) guardada(s)${
        cleared ? `, ${cleared} borrada(s)` : ""
      }`,
    });
  } catch (err: any) {
