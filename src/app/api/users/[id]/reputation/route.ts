...
import { getUserById, findListings } from "@/lib/db-raw";
...
export async function GET(
  ...
  try {
    ...
    // Original Prisma query selected only { rating, reviewCount } — order
    // doesn't matter for aggregation, so we use the default sort.
    ...
    const totalScore =
      ratingScore + verifiedScore + planScore + reviewScore + ageScore;
