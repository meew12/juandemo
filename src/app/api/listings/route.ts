...
export async function POST(req: Request) {
  try {
    ...
    let userMessage = "Error interno";
    let statusCode = 500;

    if (errMsg.includes("SQLITE_TOOBIG") || errMsg.includes("too big") || errMsg.includes("too large")) {
      userMessage = "Las imágenes son demasiado grandes para guardar. Subí imágenes más pequeñas (máx 3MB total).";
      statusCode = 413;
    } else if (errMsg.includes("body size") || errMsg.includes("payload too large") || errMsg.includes("413")) {
      userMessage = "La publicación es demasiado grande. Reducí la cantidad o tamaño de las imágenes.";
      statusCode = 413;
    } else if (errMsg.includes("UNIQUE constraint") && errMsg.includes("slug")) {
      userMessage = "Error: el título ya existe. Probá con otro título.";
      statusCode = 409;
    } else if (errMsg.includes("FOREIGN KEY")) {
      userMessage = "Error: la categoría seleccionada no es válida.";
      statusCode = 400;
    } else if (errMsg.includes("No autenticado") || errMsg.includes("session")) {
      userMessage = "Tu sesión expiró. Volvé a iniciar sesión.";
      statusCode = 401;
    }

    return NextResponse.json({ error: userMessage, details: process.env.NODE_ENV === "development" ? errMsg : undefined }, { status: statusCode });
  }
}
