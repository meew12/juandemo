// ════════════════════════════════════════════════════════════
//  UMPI - NextAuth Configuration
//  Versión AUTOCONTENIDA: no depende de Prisma ni de db-raw.ts
//  Usa @libsql/client directo para evitar "URL_INVALID" en Vercel
// ════════════════════════════════════════════════════════════
...
// ─── Cliente libsql directo (bypassa Prisma) ───────────────────
...
function getDbClient(): Client {
  ...
  if (!databaseUrl || databaseUrl === "undefined") {
    console.warn("[auth] ⚠️ DATABASE_URL no configurada, usando SQLite local");
    _client = createClient({ url: "file:./db/custom.db" });
    return _client;
  }

  // Extraer authToken de la URL o de variable separada
  let url = databaseUrl;
  ...
  console.log(`[auth] Conectando a ${isTurso ? "Turso" : "SQLite"}: ${url}`);
  console.log(`[auth] authToken: ${authToken ? "***presente***" : "(ninguno)"}`);
...
// ─── Helpers de DB (usan libsql directo) ───────────────────────

async function findUserByEmail(email: string) {
  const client = getDbClient();
  const result = await client.execute({
    sql: `SELECT id, email, name, lastName, passwordHash, image, role, plan, banned, verified
